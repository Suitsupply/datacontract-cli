...
def get_dimension(data_contract_field: Field):
    ...
    field_config = FieldConfigDBT.model_validate(data_contract_field.config)
    dimension_type = 'dimension'

    field_name = data_contract_field.title
    field_sql_name = field_name
    field_type = field_config.bigqueryType
    
    lookml_type = FIELD_TYPE_MAPPING.get(field_type, "string")
    lookml_type_transform = FIELD_TYPE_TRANSFORM.get(field_type, ["",""])

    field_description = data_contract_field.description
    field_description = field_description.rstrip().replace('"', "'") if field_description else ''

    primary_key = None
    if field_config.meta.get('primary_key', False):
        primary_key = "yes"
    
    hidden = None
    if field_config.meta.get('hidden', False):
        hidden = "yes"
    
    group_label = field_config.meta.get('group_label', None)
...
def looker_base_view_lkml(data_contract_model: Model, override_dataset_id: str = None):
    ...
    for field_key, field in data_contract_model.fields.items():
        ...
        dimension_type, field = get_dimension(field)
    ...
    source_data = {
        "views":[{
            "name": model_config.extended.entity_label
            ,"sql_table_name": f"`{model_config.extended.source_project}.{override_dataset_id.dataset if override_dataset_id else model_config.extended.source_dataset}.{model_config.extended.entity_label}`"
